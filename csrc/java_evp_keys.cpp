    ...
    JNIEnv* pEnv, jclass, jlong keyHandle)
{
    ...
    try {
        ...
        if (!result) {
            throw_java_ex(EX_OOM, "Unable to allocate raw private key array");
        }
        env->SetByteArrayRegion(result, 0, keyLen, (jbyte*)keyBuffer.get_buffer());
    } catch (java_ex& ex) {
        ex.throw_to_java(pEnv);
    }
    ...
    JNIEnv* pEnv, jclass, jlong keyHandle)
{
    ...
    try {
        ...
        result = env->NewByteArray(keyLen);
        if (!result) {
            throw_java_ex(EX_OOM, "Unable to allocate raw public key array");
        }
        env->SetByteArrayRegion(result, 0, keyLen, (jbyte*)keyBuffer.get_buffer());
    } catch (java_ex& ex) {
        ex.throw_to_java(pEnv);
